...
# Chargement du modèle selectionné

model=joblib.load(filename="Modèle de prédiction CatBoostClassifier")


# Définition de la fonction qui retourne la prédiction
...
if st.button("Predict"):
    predi = prediction(amt_credit, amt_annuity, days_birth, days_employed, days_registration, days_id_publish,
                       ext_source_2, ext_source_3, days_last_phone, amount_previous_credit)
    ...
    for i, col in enumerate(['AMT_CREDIT', 'AMT_ANNUITY', 'DAYS_BIRTH', 'DAYS_LAST_PHONE_CHANGE']):
        ...
        axes[row, col_idx].scatter(client_input[col], 1, marker='X', color='red', label='Client Testé')
...
#     # Charge de l'ensemble des clients 
#     all_clients_df = pd.read_csv("Ensemble_des_clients.csv") 

#     # Sélection des colonnes pour la comparaison
#     columns_for_comparison = ['AMT_CREDIT', 'AMT_ANNUITY', 'DAYS_BIRTH','DAYS_LAST_PHONE_CHANGE']
                            
    
#     # Ajout des colonnes d'entrée pour le client testé
#     client_input = pd.DataFrame([[amt_credit, amt_annuity, days_birth, days_last_phone]], columns=columns_for_comparison)


#     # Scatter plot pour comparer le client testé à l'ensemble des clients
#     st.subheader("Comparaison du client testé à l'ensemble des clients")

#     # Sélection d'un échantillon aléatoire pour éviter les surcharges visuelles
#     sampled_clients = all_clients_df.sample(1000)

#     # Scatter plot avec les colonnes choisies
#     fig, axes = plt.subplots(nrows=2, ncols=2, figsize=(12, 10))

#     # Utilisation d'une forme différente pour le client testé
#     for i, col in enumerate(columns_for_comparison):
#         row, col_idx = divmod(i, 2)

#         # Utilisation de col_idx pour le scatter plot
#         axes[row, col_idx].scatter(sampled_clients.index, sampled_clients[col], alpha=0.5, label='Autres clients')
#         axes[row, col_idx].scatter(client_input.index, client_input[col], marker='X', color='red', label='Client Testé')
...
# # Lien de déploiement streamlit https://karima-t-touhtouhkarima-streamlitprojet7-pbo9fw.streamlit.app/